...
-- NO_SWAP  (Addr 34 = 0x22)
 ...
 35 => "10100000", -- MOV A, E          A = i (FIX: Index i vor dem SUB in ein Arbeitsregister kopieren!)

-- i <= D? → ALU SUB D,A → A = D-i. Sign wenn D<i → Schleife fertig
 36 => "11001000", -- ALU SUB          (D - i)
 37 => "00011000", -- Byte2: SrcA=D(011), SrcBDst=A(000)  → 00_011_000
-- Sign=1 wenn D<i  → Schleife weiter (i<=D → kein Sign → weiter)
-- JCC !sign → INNER_LOOP (Addr 6)
 38 => "11111110", -- JCC !sign        11_111_110  (y=111=!sign)
 39 => "00000110", -- L = 0x06          (Little-Endian!)
 40 => "00000000", -- H = 0x00

-- Ende innere Schleife — swap-flag prüfen
-- ALU SUB C,B: erst B=0 laden, dann C-B prüfen (zero wenn C=0)
 41 => "01001100", -- LDR B, 0         B = 0 als Vergleichswert
 42 => "00000000",
 43 => "11001000", -- ALU SUB          C - B(=0) → Ergebnis→B, Zero wenn C=0
 44 => "00010001", -- Byte2: SrcA=C(010), SrcBDst=B(001)  → 00_010_001
-- JCC zero → DONE (Addr 61 = 0x3D durch Adressverschiebung)
 45 => "11000110", -- JCC zero         11_000_110
 46 => "00111101", -- L = 0x3D = 61    (Little-Endian!)
 47 => "00000000", -- H = 0x00

-- D-- und D<0 prüfen
 48 => "01011001", -- DCR D            D--  (01_011_001)
-- ALU SUB D,B (B=0 noch gesetzt) → Sign wenn D unter 0 fällt (0xFF)
 49 => "11001000", -- ALU SUB          D - 0
 50 => "00011001", -- Byte2: SrcA=D(011), SrcBDst=B(001)  → 00_011_001
-- JCC sign → DONE (Addr 61 = 0x3D)
 51 => "11110110", -- JCC sign         
 52 => "00111101", -- L = 0x3D = 61    (Little-Endian!)
 53 => "00000000", -- H = 0x00
...
-- DONE  (Addr 61 = 0x3D)
 61 => "01101100", -- LDR H, 1
 62 => "00000001",
 63 => "01110100", -- LDR L, 0         L = 0x00  (01_110_100)
 64 => "00000000",
 65 => "01000110", -- LOAD A, [H:L]     A = arr[0] (kleinster Wert)
 66 => "00000001", -- OUT              (00_000_001)
 67 => "00000000", -- Register A = 0x00
-- Tight loop (Endlosschleife auf neue Adresse 68 = 0x44)
 68 => "00000010", -- JMP → 68 (selbst)
 69 => "01000100", -- L = 0x44 = 68    (Little-Endian!)
 70 => "00000000", -- H = 0x00
